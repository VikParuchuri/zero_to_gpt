...
class Conv(Module):
    def __init__(self, input_channels, output_channels, kernel_x, kernel_y, relu=True):
        self.add_relu = relu
    ...
    def backward(self, grad, lr):
        ...
        for channel in range(self.input_channels):
            ...
            flat_input = unroll_image(self.input[channel, :], grad_x, grad_y)
            for next_channel in range(self.output_channels):
                # Kernel update
        ...
        for next_channel in range(self.output_channels):
            ...
            for channel in range(self.input_channels):
                ...
                updated_grad = convolve(flat_padded, flipped_kernel).reshape(self.input.shape[1], self.input.shape[2])
